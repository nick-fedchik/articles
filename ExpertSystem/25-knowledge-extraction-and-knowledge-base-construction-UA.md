...
> **Наступна стаття:** [26 — Інженерний граф знань (EKG): інтеграція вимог, вихідного коду, тестів та апаратури в єдиний простір трасування](26-engineering-knowledge-graph-traceability-UA.md)  
...
Розглянемо чотири класи практичних інженерних завдань, які успішно вирішуються завдяки скомпільованій базі знань.

### 1. Автоматична причинно-наслідкова діагностика та дерево пояснення (Causal Diagnostics)

Якщо у випробуваннях прототип пристрою чи мережевий сервіс повертає помилку, традиційний пошук причини (*Root Cause Analysis*) вимагає годин ручного гортання специфікацій.

Маючи скінченний автомат у базі знань, експертна система не просто повертає числовий код або вердикт відмови, а будує структуроване **дерево пояснення (Explanation Tree)**:

```math
\mathcal{T}_{\mathrm{explanation}} = \langle \mathrm{Claim}, \mathrm{RootCause}, \mathrm{ViolatedPrerequisite}, \mathrm{ValidTransitions}, \mathrm{RemediationPath}, \mathcal{E} \rangle
```

де:

- $\mathrm{Claim}$ — чітке формулювання діагнозу (наприклад, неможливість виконання команди $C$ у стані $S$).
- $\mathrm{RootCause}$ — вичерпне пояснення суті порушення норми з посиланням на конкретний пункт стандарту.
- $\mathrm{ViolatedPrerequisite}$ — ідентифікація пропущеного кроку життєвого циклу (наприклад, відсутність попередньої автентифікації чи неініціалізована сесія обміну).
- $\mathrm{ValidTransitions}$ — перелік команд, дозволених стандартом у поточному стані.
- $\mathrm{RemediationPath}$ — впорядкована послідовність дій для виходу з аварійного стану та коректного досягнення бажаної мети.
- $\mathcal{E}$ — побайтові докази першоджерела.

Такий підхід дозволяє системі відповідати на контрфактичні інженерні запитання: *«Чому команда $C$ завершилася помилкою?»* або *«Що станеться, якщо надіслати подію $E$ до завершення фази ініціалізації?»*.

### 2. Синтез валідних сценаріїв взаємодії (Solution Synthesis)

При проєктуванні клієнтських модулів, драйверів периферії чи інтерфейсних адаптерів постає завдання синтезу коректної сесії спілкування з цільовою системою.

Замість написання коду вручну експертна система розв'язує задачу досяжності цільового стану на графі FSM:

```math
s_0 \xrightarrow{e_1} s_1 \xrightarrow{e_2} s_2 \dots \xrightarrow{e_k} s_{\mathrm{target}}
```

Система знаходить найкоротший шлях переходів, підставляє валідні аргументи, виведені з граматик ABNF/EBNF, і синтезує готовий артефакт: інтерактивну сесію взаємодії або автоматизований сценарій виконання (наприклад, скрипт Shell чи тестовий модуль мовою Python).

### 3. Синтез негативних тест-кейсів для верифікації конформантності (Negative Conformance Testing)

Для сертифікації виробу недостатньо перевірити, що він працює за ідеальних умов (*Happy Path*). Необхідно довести, що система **коректно відхиляє невалідні дії**.

Використовуючи автомат станів, експертна система здатна в автоматичному режимі синтезувати повний набір негативних тест-кейсів:

- Для кожного стану $s \in S$ вибирається команда чи сигнал $e_{\mathrm{invalid}} \notin \mathrm{ValidNextCommands}(s)$.
- Формується сценарій, у якому клієнт навмисно порушує протокол (наприклад, спроба передачі даних до конфігурації або виконання операцій з поштовою скринькою до проходження автентифікації).
- Тест перевіряє, чи повертає сервер саме той код відмови, який приписаний стандартом (наприклад, помилка послідовності `503`, відмова стану `-ERR` чи статус `400 Bad Request`), із генерацією сертифіката доведення.

### 4. Автоматична детекція колізій і регуляторний аудит
