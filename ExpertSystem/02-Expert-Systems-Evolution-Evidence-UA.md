...
Додатне відношення правдоподібностей $\mathrm{LR}^{+}=8$ означає: таке
...
```math
\frac{\Pr(H\mid E)}{1-\Pr(H\mid E)}
=\frac{\Pr(H)}{1-\Pr(H)}\cdot\mathrm{LR}^{+}.
```

Ліворуч стоять **оновлені шанси** після спостереження. Перший дріб праворуч —
**початкові шанси**, а $\mathrm{LR}^{+}$ показує силу нового сигналу. Отже:
початкові шанси множаться на силу сигналу й утворюють оновлені шанси.
...
Тут $\mathrm{Pa}(X_i)$ - батьківські вузли $X_i$ у спрямованому ациклічному графі. Розклад коректний, коли структура графа кодує відповідні умовні незалежності; кожна локальна таблиця або модель $P(X_i\mid\mathrm{Pa}(X_i))$ має бути нормованим умовним розподілом.
...
\mathrm{BM25}(q,d)
=\sum_{t\in q}
\underbrace{\ln\!\left(1+\frac{N-n_t+0{,}5}{n_t+0{,}5}\right)}_{\mathrm{IDF}(t)}
\frac{f(t,d)(k_1+1)}
     {f(t,d)+k_1\!\left(1-b+b\frac{|d|}{\mathrm{avgdl}}\right)}.
```

Тут $f(t,d)$ - частота терма, $|d|$ - довжина документа, $\mathrm{avgdl}$ - середня довжина, а $k_1>0$ і $b\in[0,1]$ - параметри. Існують різні реалізаційні варіанти IDF, тому необроблені оцінки BM25 з різних рушіїв не слід трактувати як одну шкалу або ймовірність. Сьогодні до лексичного пошуку додалися щільні векторні представлення, але базова ідея не змінилася: знайти документи або фрагменти, які мають найбільше відношення до питання.
