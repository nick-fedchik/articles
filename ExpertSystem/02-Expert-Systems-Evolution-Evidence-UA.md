...
нечітка належність, пошуковий score і впевненість мовної моделі — різні
величини. Якщо змішати їх в одне число, система створить видимість точності,
але не дасть перевірюваного рішення.

Це навчальний історико-технічний огляд для розробника, а не повний курс
теорії ймовірностей або формальна safety-, medical- чи legal-оцінка. Кожну
...
flowchart LR
    ...
    T -->|"точне число"| D["Детерміноване ядро"]
    R --> P["Пояснюваний пакет доказів"]
    B --> P
    F --> P
    G --> P
    D --> P
...
$$
\underbrace{\Pr(H\mid E)}_{\text{апостеріорна ймовірність}}
=
\frac{
  \underbrace{\Pr(E\mid H)}_{\text{правдоподібність}}
  \underbrace{\Pr(H)}_{\text{апріорна ймовірність}}
}{
  \underbrace{\Pr(E)}_{\text{повна ймовірність свідчення}}
},
\qquad \Pr(E)>0.
$$
...
Тут $f(t,d)$ - частота терма, $|d|$ - довжина документа, $\operatorname{avgdl}$ - середня довжина, а $k_1>0$ і $b\in[0,1]$ - параметри. Існують різні реалізаційні варіанти IDF, тому сирі BM25-score з різних рушіїв не слід трактувати як одну шкалу або ймовірність. Сьогодні до лексичного пошуку додалися щільні векторні представлення, але базова ідея не змінилася: знайти документи або фрагменти, які мають найбільше відношення до питання.
...
| Домен | Де корисне оновлення впевненості | Типова помилка | Межа рішення |
|---|---|---|---|
| Automotive | діагностика відмов, оцінювання польового ризику, вибір наступного тесту | перетворити частоту дефектів іншої конфігурації на апріорну ймовірність поточного виробу | модель пріоритезує перевірку; safety case, release і ремонтне рішення мають визначених відповідальних |
| Aviation | maintenance diagnosis, аналіз несправностей і аномалій, підтримка safety assessment | вважати корельовані сенсори незалежними або переносити статистику між fleet/configuration без перевірки | імовірнісний висновок є advisory evidence; approved data, assurance і повноваження людини залишаються визначальними |
| Medicine | оновлення діагностичної гіпотези після тесту, оцінка ризику для визначеної популяції | плутати $\Pr(E\mid H)$ із $\Pr(H\mid E)$ і не врахувати prevalence, calibration чи population shift | результат підтримує клінічне рішення в межах intended use; він не підміняє клініциста й застосовні правила для CDS/software as a medical device |
| Military/Defence | об'єднання невизначених повідомлень, технічна діагностика, readiness і логістика | повторно порахувати залежні джерела, не врахувати deception, classification або деградацію сенсорів | система показує provenance, конфлікт і невизначеність; command authority та правова оцінка не переходять до моделі |
| Legislation | ранжування релевантних норм і справ, оцінка ймовірності того, що пошук пропустив важливе джерело, планування додаткової перевірки | видати статистичну релевантність або прогноз за юридичну чинність норми | чинність визначають юрисдикція, дата, редакція, ієрархія джерел і компетентна особа; ймовірність лише керує пошуком та review |

У законодавстві різниця особливо наочна. Високий retrieval-score або
...
стабільний ідентифікатор, офіційне джерело, temporal scope, редакція,
юрисдикція та зв'язки між актами. ELI підтримує ідентифікацію й структуровані
metadata законодавства, а LegalRuleML — подання юридичних норм і властивостей
...
flowchart TB
    ...
    E["Дані + provenance + версія"] --> M["Модель невизначеності"]
    M --> O["Оцінка + припущення + межі"]
    O --> A["Automotive<br>assurance / release"]
    O --> V["Aviation<br>approved process"]
    O --> H["Medicine<br>clinician / intended use"]
    O --> D["Military/Defence<br>human command"]
    O --> L["Legislation<br>competent legal authority"]
```
...
один відсутній safety approval може бути блокером незалежно від високої частки.
...
**Простий захист.** Перед тим як покласти чанк $c$ в індекс, рахуємо частку
друкованих символів:
...
Навчальне gate-правило можна записати явно:
...
треба перевіряти на розмічених випадках. Наприклад, Brier score для бінарного
результату $y_i\in\{0,1\}$ дорівнює:
...
Менше значення краще, але одна метрика не розрізняє всі причини помилки. Тому
разом перевіряють calibration curve, discrimination, окремі зрізи даних і
зміну розподілу. Пошуковий score, certainty factor або компоненти
...
ризиків. Сьогодні без явних ingestion gates, версій і декомпозиції
