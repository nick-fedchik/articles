# Як експертна система розуміє текст і не втрачає джерело відповіді
...
> **Після статті:** пояснювати шлях від фрази користувача до цитати в документі, розрізняти схожість і доказ та перевіряти локальну модель перед оновленням.

Користувач запитує: «Яка мінімальна довжина заголовка?» У документі відповідь
записано як `eight octets`. Система може знайти це речення, але все ще
помилитися: перетворити «октети» на «байти» без перевірки, загубити одиницю або
цитувати іншу версію документа. Отже, розуміння мови починається не з великої
моделі, а з уміння не втратити зміст і походження тексту.

У нашому прикладі правильна відповідь — не голе `8` і не автоматично «8 байтів».
Вона містить значення, оригінальну одиницю, умову, за якої правило чинне, та
точний фрагмент дозволеної версії документа. Саме цей повний ланцюжок, а не
красиве перефразування, робить відповідь експертної системи перевірюваною.

Надійна мовна частина системи є гібридною: детермінований код відповідає за
байти, координати й правила доступу; аналізатор пропонує форму та будову
речення; моделі знаходять схожі фрагменти й перевіряють вузькі твердження;
мовна модель допомагає перефразувати питання. До відповіді потрапляє лише те,
що окрема перевірка може відтворити з доказу.
...
prediction мають різні підстави. Мовна частина системи з’єднує ці рівні.
...
Кожна стрілка створює новий текст і відношення походження. Нехай
...
Текст може виглядати для людини звичайним, але по-різному читатися програмами
або містити приховану команду для мовної моделі. Тому перед пошуком відповіді
система мусить перевірити не тільки зміст документа, а й спосіб, у який його
закодовано та показано.
...
Ці перевірки не роблять документ безпечним самі по собі. Вони дають людині й
програмі змогу побачити нестандартну форму тексту; правила доступу та перевірка
доказів окремо вирішують, чи можна використати його у відповіді.

## Як розподілити відповідальність між складниками

Жоден компонент не повинен одночасно знаходити текст, тлумачити його і сам
дозволяти власний висновок. Поділ ролей потрібен, щоб при помилці було видно:
загубили байти, хибно розібрали речення, обрали не той фрагмент чи неправильно
підвищили статус твердження.
...
Такий поділ дозволяє замінювати модель без зміни правил доказу. Якщо
перефразування працює гірше, система має відмовитися або поставити уточнювальне
питання, але не створити нову цитату чи обійти обмеження доступу.

## Скільки тексту може опрацювати модель
...
## Коли локальна модель справді корисна

Локальне виконання корисне, коли дані не можна передавати назовні, зв’язок
ненадійний або час відповіді важливий. Водночас слово «локальна» описує місце
обчислення, а не гарантує точність, відтворюваність чи відсутність мережевого
обміну.
...
## Як оновлювати модель без непомітної втрати якості
...
## Як чесно перевірити мовну систему
...
## Як оцінити повноту пошуку й точність відповіді
...
Якість відповіді не можна відокремити від ціни її отримання. Після перевірки
змісту команда має окремо побачити, чи вкладається система в потрібний час,
пам’ять і енергетичний бюджет, не приховуючи затримку в черзі.

## Як вимірювати швидкість і витрати
...
## З чого почати першу робочу версію

Спочатку зберігають незмінні байти джерела, рішення про кодування та хеш
вмісту. Далі будують карту зв’язку з джерелом, перевіряють Unicode і додають
словник предметної області для чисел, одиниць, ідентифікаторів, префіксів і
хешів. Це дає основу, без якої наступні моделі не зможуть виправити втрату
точного фрагмента.

Потім створюють простий пошуковий базовий варіант, а вже після нього додають
багатомовну модель для пошуку й повторного ранжування. Контракт запиту та
пропозиція твердження мають бути структурованими; синтаксичний аналізатор або
малу мовну модель підключають лише до класу помилок, який справді виміряли.
Якщо модель не може однозначно повернутися до байтів, вона повинна утриматися
від відповіді.

Нарешті калібрують перевірку тверджень, заморожують маніфест, оцінюють втрату
якості після стискання моделі та права на використання складників. Нову версію
порівнюють із попередньою на відкладених даних, у тіньовому режимі й на
обмеженому випуску; лише після цього її можна ввімкнути атомарно.
...
Після цієї статті читач може простежити шлях від запиту до точного фрагмента
джерела, пояснити, чому високий показник схожості не доводить твердження, та
вимагати від локальної моделі маніфесту, перевірки й можливості відкату. Саме
ці три вміння дозволяють користуватися мовними моделями без перетворення їхніх
припущень на неперевірене знання.
