...
> **Серія:** [Експертні системи для R&D](README.md) · стаття 04 із 22  
...
## Предикативна логіка, Datalog та багатозначні системи

Продукційні правила над пропозиційними символами виду $P \land Q \Rightarrow R$ швидко вичерпують себе у складних інженерних завданнях: неможливо створювати окреме правило для кожного конкретного регістра, піна мікроконтролера чи коду помилки. Справжня експертна система переходить від логіки висловлювань до **логіки предикатів першого порядку (First-Order Logic, FOL)** та її обчислюваних фрагментів.

У логіці першого порядку твердження виражаються через предикатні відношення $P(t_1, t_2, \dots, t_n)$ над термінами, константами та змінними. Це дозволяє формулювати універсальні інженерні закони через квантори загальності ($\forall$) та існування ($\exists$):

```math
\forall x \, \Big( \mathrm{SafetyCritical}(x) \land \mathrm{Modified}(x) \implies \exists y \, \big( \mathrm{Verifies}(y, x) \land \mathrm{Verdict}(y, \mathrm{PASS}) \big) \Big)
```

Ця формула читається однозначно: *«Для будь-якого компонента $x$, якщо він критичний для безпеки і був змінений, обов'язково існує валідуючий тест $y$, чий статус виконання є успішним»*.

### Декларативний Datalog: безпека та фіксована точка

Повна логіка першого порядку є нерозв'язною (теорема Чьорча-Тюрінга). Тому в інженерних експертних системах використовують **Datalog** — мову декларативних запитів на основі Horn-диз'юнктів без функціональних символів, що гарантує завершуваність обчислень за поліноміальний час (*PTIME data complexity*).

Правило в Datalog має вигляд:

```math
H(X, Y) \leftarrow B_1(X, Z), B_2(Z, Y), \dots, B_k(Z_k, Y).
```

де $H$ — голова (новий факт), а $B_i$ — підтверджені атоми тіла правила. Семантика Datalog визначається через оператор безпосередніх наслідків $T_P$, який ітеративно розширює множину фактів до досягнення **найменшої нерухомої точки (Least Fixed Point, LFP)**:

```math
T_P(I) = I \cup \{ A \mid A \leftarrow B_1, \dots, B_k \text{ є правилом, і } B_1, \dots, B_k \subseteq I \}
```

```math
\mathrm{LFP}(T_P) = \bigcup_{i=0}^{\infty} T_P^i(\emptyset)
```

Для обробки відсутності фактів використовується **стратифіковане заперечення** ($\mathrm{not}$): граф залежностей між правилами ділиться на рівні (страти), що виключає парадокси самозаперечення на зразок $P \leftarrow \mathrm{not} \, P$.

### Тризначна логіка Кліні (Kleene Strong 3-Valued Logic)

У реальному R&D діє принцип неповноти: знання про світ ніколи не є вичерпним. Класична двозначна логіка припускає, що твердження обов'язково або істинне, або хибне. У системах критичного призначення це веде до катастрофи: якщо факт не знайдено, наївна система вважає його хибним (Closed-World Assumption).

Натомість надійна експертна система спирається на **сильну тризначну логіку Кліні** із множиною значень істинності $\mathcal{V}_3 = \{\mathrm{True}\ (1), \mathrm{Unknown}\ (1/2), \mathrm{False}\ (0)\}$:
...
| **Unknown** | **False** | False | Unknown | Unknown | True |
| **False** | **False** | False | False | True | True |

Користь логіки Кліні для інженера: якщо вхідна вимога не знайдена в нормативній базі, система обчислює статус як **Unknown**, що породжує типізовану відмову (`KindQualifiedNonanswer`), а не хибне ствердження про відсутність обмежень.

### Деонтична модальна логіка інженерних стандартів

Нормативні стандарти оперують не просто фактами, а модальностями поведінки. **Деонтична логіка** моделює поняття обов'язку, заборони та дозволу:

```math
\mathcal{O}(p) \colon \text{обов'язково } p\ (\text{SHALL / MUST})
```

```math
\mathcal{F}(p) \colon \text{заборонено } p\ (\text{SHALL NOT / MUST NOT})
```

```math
\mathcal{P}(p) \colon \text{дозволено } p\ (\text{MAY / OPTIONAL})
```

Ці модальності пов'язані дуальними аксіомами:

```math
\mathcal{O}(p) \equiv \neg \mathcal{P}(\neg p), \qquad \mathcal{F}(p) \equiv \mathcal{O}(\neg p) \equiv \neg \mathcal{P}(p)
```

Якщо при об'єднанні двох різних галузевих специфікацій предикатний рушій виводить:

```math
\mathcal{O}(p) \land \mathcal{F}(p) \vdash \bot
```

експертна система фіксує неусувну нормативну колізію (логічну суперечність) і блокує схвалення проєкту до усунення розбіжностей регуляторами.

---

## Тріада міркувань за Пірсом: дедукція, індукція та редукція

У процесі дослідження та проєктування складних систем інженер комбінує фундаментальні форми умовиводів. Американський логік і філософ Чарльз Сандерс Пірс формалізував три взаємодоповнюючі режими міркування через взаємозв'язок між **Правилом** ($\mathrm{Rule} \colon P \to Q$), **Випадком/Засновком** ($\mathrm{Case} \colon P$) та **Результатом** ($\mathrm{Result} \colon Q$).

```mermaid
flowchart TD
    accTitle: Тріада форм міркування за Чарльзом Пірсом
    accDescr: Схема взаємозв'язку між дедукцією, індукцією та діагностичною абдукцією через правило, засновок та результат.

    subgraph Deduction["Дедукція (Deduction)"]
        D1["<b>Правило:</b> P → Q<br/><b>Засновок:</b> P"] --> D2["<b>Результат:</b> Q<br/>(100% гарантія істинності)"]
    end

    subgraph Induction["Індукція (Induction)"]
        I1["<b>Засновок:</b> P(a₁)...P(aₖ)<br/><b>Результат:</b> Q(a₁)...Q(aₖ)"] --> I2["<b>Гіпотеза правила:</b> ∀x (P(x) → Q(x))<br/>(Емпіричне узагальнення)"]
    end

    subgraph Abduction["Абдукція (Abduction / Diagnosis)"]
        A1["<b>Правило:</b> P → Q<br/><b>Результат (симптом):</b> Q"] --> A2["<b>Гіпотетична причина:</b> P<br/>(Пояснення відмови)"]
    end
```

### 1. Дедукція (Deduction): збереження істинності

Дедукція рухається від загального до конкретного за правилом *modus ponens*:

```math
\frac{P \implies Q, \quad P}{Q}
```

- **Властивість:** дедуктивний висновок гарантовано істинний, якщо істинними є засновки. Вона не породжує нових знань поза межами вихідних аксіом, але забезпечує абсолютну простежуваність.
- **Інженерне застосування:** перевірка відповідності коду стандарту, автоматизований розрахунок покриття вимог, виконання інваріантів автоматів станів. Якщо стандарт вимагає, що відправка команди без автентифікації повертає помилку 503, і клієнт не виконав автентифікацію, дедуктивний рушій гарантує вердикт 503.

### 2. Індукція (Induction): емпіричне узагальнення

Індукція рухається від окремих фактів до формулювання загального правила:

```math
\frac{P(a_1) \land Q(a_1), \quad P(a_2) \land Q(a_2), \quad \dots, \quad P(a_m) \land Q(a_m)}{\forall x \, \big(P(x) \implies Q(x)\big)}
```

- **Властивість:** індуктивне правило є **гіпотезою**. Воно розширює знання, але несе ризик помилки (індуктивна проблема «чорного лебедя»).
- **Інженерне застосування:** вилучення граматик ABNF та автоматів станів FSM зі зразків протокольних сесій; виявлення статистичних часових затримок перемикання вентилів. В експертних системах суворого призначення індуктивні правила ніколи не потрапляють у виробничий контур автоматично — вони маркуються як кандидати і вимагають верифікації людиною-експертом.

### 3. Діагностична абдукція (Abduction)

Абдукція рухається від спостережуваного результату (симптому) до найбільш вірогідної причини:

```math
\frac{P \implies Q, \quad Q}{\text{Гіпотеза: } P}
```

- **Властивість:** абдукція логічно неповноцінна (оскільки $Q$ могло бути викликане не лише $P$, а й іншою причиною $R$). Проте це єдиний математичний апарат генерації діагностичних гіпотез.
- **Інженерне застосування:** встановлення першопричини аварії (Root Cause Analysis). Якщо в логах зафіксовано тахометричний збій ($Q$), абдуктивний аналізатор генерує мінімальний набір гіпотез $P_1, P_2$, які перевіряються подальшими тестами.

### 4. Редукція (Reduction): приборкання комбінаторного вибуху

У складних інженерних системах розмірність задачі часто унеможливлює прямий розрахунок. **Редукція** — це зведення складної задачі до простішої або канонічної форми зі збереженням істотних властивостей.

Математично розрізняють три типи редукції:

1. **Редукція проблеми (Problem Reduction):** рекурсивне розбиття цільового твердження через дерево цілей (AND/OR graphs). Доведення складної мети $\mathcal{G}$ зводиться до кон'юнкції або диз'юнкції підцілей:

   ```math
   \mathcal{G} \equiv \bigwedge_{i=1}^k g_i \quad \lor \quad \bigvee_{j=1}^m h_j
   ```

2. **Редукція простору станів (State-Space Reduction):** мінімізація скінченних автоматів станів за алгоритмом Гопкрофта шляхом розбиття множини станів $\mathcal{S}$ на класи еквівалентності за $O(n \log n)$ кроків:

   ```math
   s_1 \sim s_2 \iff \forall w \in \Sigma^* \colon \delta^*(s_1, w) \in \mathcal{F} \iff \delta^*(s_2, w) \in \mathcal{F}
   ```

   Це дозволяє скоротити автомат протоколу з тисяч комбінаторних станів до канонічного компактного ядра.

3. **Криптографічна редукція свідчень (Merkle Proof Reduction):** зведення довільного масиву із $N$ свідчень і цитат до одного 32-байтного кореневого хешу дерева Меркла $R = \mathrm{Root}(\mathcal{T}_{\mathrm{Merkle}})$. Будь-який окремий факт доводиться аудитору за логарифмічний час $O(\log N)$ через вектор сусідніх хешів (*Audit Path*), не вимагаючи розкриття всієї бази знань.

| Режим міркування | Засновки | Висновок | Гарантія істинності | Сфера застосування |
|---|---|---|---|---|
| **Дедукція** | Правило + Засновок | Наслідок | 100% (Sound) | Сертифікація, аудит, виконання FSM |
| **Індукція** | Засновок + Наслідок | Гіпотеза правила | Ймовірнісна | Синтез граматик, навчання на тестах |
| **Абдукція** | Правило + Наслідок | Гіпотеза засновку | Евристична | Діагностика збоїв, пошук першопричини |
| **Редукція** | Складна модель/граф | Канонічна підмодель | Еквівалентність за структурою | Мінімізація FSM, Merkle Tree докази |
...
```math
\mathrm{sim}(q,c)=\frac{\sum_{i=1}^{m}w_i\mathrm{sim}_i(q_i,c_i)}{\sum_{i=1}^{m}w_i},
\qquad w_i\geq 0,\quad \sum_{i=1}^{m}w_i>0,
```

де $q$ — новий випадок, $c$ — історичний, $\mathrm{sim}_i$ порівнює одну ознаку, а $w_i$ показує її важливість. Наприклад, для дефекту вага спільного компонента може бути більшою за схожість формулювання задачі. Користь формули — зробити причину збігу перевірюваною; число схожості не є ймовірністю правильності рішення.
...
```math
S(a)=\sum_{i=1}^{m}w_i r_i(a),\qquad w_i\geq 0,\quad \sum_{i=1}^{m}w_i=1,
```
...
```math
\cos(u,v)=\frac{u\cdot v}{\lVert u\rVert\lVert v\rVert},\qquad u\neq 0,\quad v\neq 0.
```
...
```math
e_{t_i}=\mathrm{onehot}(t_i)^\top E.
```

$\mathrm{onehot}(t_i)$ — вектор із одиницею в позиції токена $t_i$ та нулями всюди інше; множення просто вибирає потрібний рядок із $E$. Далі модель додає позиційну інформацію $p_i$, бо без неї «тест пройшов після виправлення» і «виправлення пройшло після тесту» були б лише набором тих самих токенів:
...
$I_b$ — група передбачень із близькою заявленою впевненістю, $\mathrm{acc}$ — фактична частка правильних, $\mathrm{conf}$ — середня заявлена впевненість, $n$ — кількість прикладів. Менше ECE зазвичай краще, але метрику оцінюють на відкладених даних і разом із reliability diagram; одна мала цифра не гарантує надійність у рідкісних або нових сценаріях.
