...
## Дворежимна архітектура: строгий аудит проти розширеного консультативного режиму

У реальному інженерному життєвому циклі виникає фундаментальна суперечність між двома сценаріями використання експертної системи:

1. **Сертифікаційний аудит (Strict Verification Mode):** вимагає безкомпромісного дотримання інваріантів *Evidence-Grounded* та *Fail-Closed*. Якщо факт не зафіксований у базі знань або є неоднозначним, система зобов'язана повернути типізовану відмову (`KindRefusal`). Жодні правдоподібні припущення чи довільні здогадки неприпустимі в авіоніці (DO-178C) чи автомобільній безпеці (ISO 26262).
2. **Дослідницький інжиніринг (Extended Advisory / Speculative Mode):** на стадії проєктування, аналізу концепцій чи налагодження нестандартних збоїв інженеру часто потрібна евристична допомога системи. Інженер запитує: *«Який типовий тайм-аут зазвичай обирають у споріднених протоколах?»* або *«Якщо ми припустимо, що живлення датчика не просідає нижче 3.3В, чи можливий перехід у стан збою?»*.

Спроба змішати ці два режими в один недиференційований потік веде до дискредитації системи: аудитори втрачають довіру до висновків, якщо поруч з нормативними фактами з'являються стохастичні здогадки.

Розв'язанням є **дворежимна архітектура з явною демаркацією знання та припущень**.

```mermaid
flowchart TD
    accTitle: Дворежимна архітектура виведення експертної системи
    accDescr: Розділення результатів на строгий детермінований висновок із цитатами та розширені спекулятивні гіпотези.

    Query["<b>Інженерний запит</b><br/>Inquiry q"] --> Core["<b>Строге ядро дедукції</b><br/>FactBase + FSM + Datalog"]
    Core --> Strict["<b>Строгий результат (StrictResult)</b><br/>• Перевірені цитати [start, end]<br/>• Або типізована відмова (Refusal)"]

    Query --> ModeCheck{"<b>Режим запиту?</b><br/>--extended"}
    ModeCheck -->|"Ні (Strict)"| OutStrict["<b>Фінальна відповідь</b><br/>Тільки сертифіковані факти"]
    ModeCheck -->|"Так (Extended)"| SpecEngine["<b>Генератор гіпотез</b><br/>Індукція + Дедуктивне розширення + CBR"]

    SpecEngine --> Hyp1["<b>Індуктивна гіпотеза</b><br/>Узагальнення з інших протоколів"]
    SpecEngine --> Hyp2["<b>Дедуктивне розширення</b><br/>Проєкція за умовних припущень"]
    SpecEngine --> Hyp3["<b>Прецедентна аналогія</b><br/>Рішення з діагностичної пам'яті"]

    Strict --> OutExtended["<b>Розширений звіт</b><br/>StrictResult + SpeculativeHypotheses<br/>(EvidenceGrounded: false)"]
    Hyp1 --> OutExtended
    Hyp2 --> OutExtended
    Hyp3 --> OutExtended
```

### Формальна модель розширеного виводу

У розширеному режимі відповідь системи являє собою кортеж із двох суворо розмежованих компонентів:

```math
\mathcal{Y}_{\mathrm{extended}}(q) = \big\langle \mathcal{T}_{\mathrm{strict}}(q), \quad \mathcal{H}_{\mathrm{speculative}}(q) \big\rangle
```

де:

- $\mathcal{T}_{\mathrm{strict}}(q)$ — канонічний результат роботи строгого ядра, де кожне ствердження підтверджене побайтовою цитатою першоджерела або повернуто формальну відмову;
- $\mathcal{H}_{\mathrm{speculative}}(q) = \{h_1, h_2, \dots, h_m\}$ — множина запропонованих гіпотез, кожна з яких має обов'язковий метаданий маркер $\mathrm{EvidenceGrounded} = \mathrm{False}$.

### Три механізми синтезу припущень

1. **Індуктивне узагальнення ($\mathcal{H}_{\mathrm{ind}}$):**
   Екстраполяція статистичних закономірностей з емпіричного досвіду подібних підсистем. Якщо стандарт явно не специфікує тайм-аут відповіді, система аналізує споріднені протоколи:

   ```math
   \mathrm{Confidence}_{\mathrm{ind}}(h) = \frac{N_{\mathrm{support}}}{N_{\mathrm{support}} + N_{\mathrm{conflict}} + \lambda}
   ```

   де $N_{\mathrm{support}}$ — кількість прецедентів, що підтверджують параметр, $N_{\mathrm{conflict}}$ — контрприклади, $\lambda > 0$ — коефіцієнт регуляризації апріорної невизначеності.

2. **Дедуктивне умовне розширення ($\mathcal{H}_{\mathrm{ded}}$):**
   Формальна проєкція наслідків, обумовлена невизначеним припущенням:

   ```math
   h_{\mathrm{ded}} \colon \mathcal{A}_{\mathrm{hypothetical}} \implies \mathcal{B}_{\mathrm{projected}}
   ```

   Система повідомляє: *«У нормативній базі відсутній факт чинності таймера T2; проте ЯКЩО припустити, що таймер T2 налаштовано на 500мс, то стан збою переповнення стає недосяжним»*.

3. **Аналогійне навчання на прецедентах ($\mathcal{H}_{\mathrm{cbr}}$):**
   Використання пам'яті минулих діагностичних розв'язань (*Case-Based Reasoning*). При схожості вектора симптомів система знаходить найближчий прецедент:

   ```math
   \mathrm{Sim}(c_{\mathrm{new}}, c_k) = \frac{\sum_{i=1}^d w_i \cdot \mathrm{sim}_i(f_i^{\mathrm{new}}, f_i^k)}{\sum_{i=1}^d w_i}
   ```

   і пропонує перевірений у минулому шлях виправлення (*Remediation Path*) як рекомендаційну гіпотезу.

### Шлюз допуску та критерії переведення гіпотези у факт (Promotion Criteria)

Кожна гіпотеза $h \in \mathcal{H}_{\mathrm{speculative}}$ містить специфікацію **критеріїв допуску (Promotion Criteria)**:

- Що саме потрібно перевірити на апаратному стенді для підтвердження.
- Який пункт стандарту чи лист правок (*errata*) необхідно знайти.
- Хто з системних експертів повинен затвердити внесення факту до основного пакета знань.

Таким чином, розширений режим не руйнує детермінізм експертної системи, а слугує контрольованим інкубатором нових знань для науково-дослідних команд.
