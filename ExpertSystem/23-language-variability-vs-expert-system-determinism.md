...
$$\text{Query}(X) \leftarrow \text{DefinitionOf}(X, \text{protocol_udp}) \wedge \text{Context}(X, \text{RFC_768})$$
...
За умови обмеженого декодування, локальний рантайм (наприклад, сумісний із **JSON Schema Draft 2020-12 Core**) на кожному кроці генерації будує динамічну логічну маску дозволених токенів \(M \subseteq V\). Ця маска визначається поточним станом синтаксичного аналізатора $$(*LL(k)*$$ або $$*LR(k)*$$ граматики). Логіти токенів, які не відповідають схемі, примусово зануляються (прирівнюються до $$\(-\infty\))$$ перед етапом обчислення ймовірностей:
