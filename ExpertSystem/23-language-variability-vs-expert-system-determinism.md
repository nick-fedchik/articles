...
## Наївний векторний пошук та наївний RAG: анатомія та обмеження підходу

**Наївний векторний пошук (Naive Vector Search)** — це базовий підхід до пошуку схожих об'єктів у векторному просторі, який полягає у прямому переборі (*exhaustive linear scan*) усіх наявних векторів у базі даних для пошуку найбільш схожих на вектор-запит за обраною метрикою відстані (найчастіше косинусною схожістю).

З алгоритмічного погляду такий метод є класичним точним пошуком найближчих сусідів ($k\text{-NN}$) з обчислювальною складністю $\mathcal{O}(N \cdot d)$, де $N$ — кількість збережених векторів, а $d$ — розмірність векторного простору (наприклад, 768 або 1536 дійсних чисел). Для невеликих масивів документів прямий перебір забезпечує стовідсоткову точність вибірки без похибок апроксимації, властивих графовим (*HNSW*) або кластеризаційним (*IVF*) індексам. Проте він діє за суто геометричним принципом, залишаючись абсолютно сліпим до логічного змісту, ієрархії вимог та взаємозв'язків між фактами.

На цьому примітиві базується так званий **наївний RAG (Naive Retrieval-Augmented Generation)** — типовий триетапний конвеєр додавання зовнішніх даних у контекст великих мовних моделей (*LLM*):

1. **Фрагментація та індексація (Chunking):** Вихідні технічні документи розрізаються на фіксовані текстові фрагменти (наприклад, по 256–512 токенів із довільним перекриттям). Розбиття виконується механічно — за кількістю слів чи символів, без урахування меж розділів, таблиць, лістингів коду чи правил специфікації. Кожен чанк перетворюється енкодером на вектор і зберігається у векторній базі даних (*VDB*).
2. **Наївне вилучення (Retrieval):** Запит інженера також транслюється у вектор. За допомогою прямого перебору чи найпростішого сканування система знаходить підмножину з $k$ векторів (*Top-k chunks*), які мають найвищу косинусну схожість із запитом.
3. **Генерація без верифікації (Generation):** Знайдені уривки конкатенуються у спільний контекст і підставляються у промпт мовної моделі. Очікується, що модель самостійно відфільтрує шум, зв'яже розрізнені факти докупи та синтезує точну відповідь.
...
В інженерних доменах R&D такий підхід спричиняє серйозні ризики:
...
$$s_{dense}(\mathbf{q}, \mathbf{d}) = \frac{\mathbf{q}^\top \mathbf{d}}{\lVert\mathbf{q}\rVert_2 \lVert\mathbf{d}\rVert_2} = \frac{\sum_{i=1}^{m} q_i d_i}{\sqrt{\sum_{i=1}^{m} q_i^2} \sqrt{\sum_{i=1}^{m} d_i^2}}$$
...
З погляду математичної логіки та інженерії знань, значення $s_{dense}(\mathbf{q}, \mathbf{d})$ є суворо **ранговим показником суміжності (ranking score)** у штучно побудованому латентному просторі моделі, а не ймовірністю логічної підтримки, істинністю чи дедуктивним доказом. Векторне кодування, виконане глибокими нейромережами (наприклад, трансформерами класу *BERT* чи двонаправленими енкодерами), фіксує статистичні закономірності спільної зустрічальності слів (*distributional semantics*), але повністю ігнорує логічні оператори, квантори та строгі предикативні зв'язки.
