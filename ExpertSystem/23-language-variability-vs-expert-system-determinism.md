...
За умови обмеженого декодування, локальний рантайм (наприклад, сумісний із規格 **JSON Schema Draft 2020-12 Core**) на кожному кроці генерації будує динамічну логічну маску дозволених токенів \(M \subseteq V\). Ця маска визначається поточним станом синтаксичного аналізатора (*LL(k)* або *LR(k)* граматики). Логіти токенів, які не відповідають схемі, примусово зануляються (прирівнюються до \(-\infty\)) перед етапом обчислення ймовірностей:
