...
Перед випуском нової версії система знаходить проблему: для релізу бракує повторного тесту безпеки. Порада зрозуміла — замовити тест. Але щойно програмі дозволяють самій бронювати стенд, змінювати статус релізу або надсилати запити, виникає інше питання: що саме вона має право змінити і як переконатися, що зміна відбулася правильно?
...
Ці три речі не можна змішувати. Мовна модель або планувальник можуть запропонувати вдалий план, але не мають самі визначати, чи можна виконати його саме зараз. У нашому прикладі система спочатку має показати доказ: який тест відсутній, для якої версії релізу, хто відповідає за перевірку і який ризик знімає повторний запуск.
...
```mermaid
flowchart LR
  E[Докази про реліз] --> R[Рекомендація]
...
Для такого релізу безпечним першим кроком може бути чернетка заявки на тест. Бронювання одного стенду на 45 хвилин — уже дія, яку варто прив’язати до схвалення. Автоматично заблокувати реліз можна лише за чітким правилом, якщо стан релізу читається з авторитетного джерела, а розблокування має окрему процедуру.
...
Для прикладу, запис має сказати не просто «створити заявку», а «зарезервувати стенд safety-3 на 45 хвилин для релізу з непідтвердженим тестом безпеки, якщо реліз досі заблокований і стенд відповідає потрібному класу безпеки». Це дозволяє людині схвалити саме те, що буде зроблено, а програмі — перевірити умови перед викликом інструмента.
...
Навіть після успішної відповіді інша частина системи має прочитати фактичний стан: чи справді існує бронювання для цього релізу, на правильний час і стенд. Це розділяє того, хто виконав дію, і того, хто засвідчує її наслідок.
...
Для оборотних дій корисно заздалегідь описати: як виявити невдачу, хто відповідає за відновлення, яку зворотну дію можна виконати і коли слід зупинитися та передати справу людині. У нашому прикладі невдала заявка не дає системі права шукати будь-який інший стенд; вона має показати проблему відповідальному інженерові.
