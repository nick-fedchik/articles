...
> **Рівень:** Knowledge Engineer / ML Engineer / технічний лід: middle+  
> **Після статті:** провести elicitation-сесію, перетворити її на versioned knowledge candidates і перевірити їх на cases, контрприкладах та незалежних джерелах.

«Запитайте нашого головного інженера — він знає систему напам'ять». Команда
записує інтерв'ю, LLM стискає transcript до двадцяти правил, а через місяць одне
з них блокує справний виріб. Експерт згадує: це правило діяло лише для старої
ревізії плати й лише за низької температури. **Проблема цієї статті — здобути
неявне знання людини разом з умовами, винятками та походженням, не перетворивши
авторитетну розповідь на неперевірену базу знань.**

Матеріал має методологічний і частково теоретичний характер. Це референсний
протокол knowledge elicitation, а не звіт про контрольований польовий
експеримент. Методи треба адаптувати до домену, ризику, права на запис і
організаційної культури; кількість сесій або «насичення» не доводить повноти.

Наскрізний приклад: senior firmware engineer діагностує рідкісний intermittent
boot failure. У документації є коди помилок, але немає ознаки, яку інженер
помічає на осцилограмі, порядку перевірок і випадку, коли звичайний power-cycle
знищує найкращий доказ. Нам треба отримати не біографічну історію, а
перевірювану модель: `cue → hypothesis → discriminating test → exception →
action`, зберігши слова й невпевненість автора.

## Документ, transcript і знання — різні речі
...
## Від фрази до knowledge candidate
...
1. **Уточнення:** що саме означає «пласка» і як це виміряти?
2. **Контраст:** який trace схожий, але веде до іншої гіпотези?
3. **Виняток:** коли ознака присутня, а clock справний?
4. **Відсутність:** що робити, якщо потрібного каналу немає?
5. **Час:** чи змінюється правило після прогріву або повторного старту?
6. **Походження:** ви це спостерігали, читали чи вивели?
7. **Незгода:** хто з колег вирішив би інакше і чому?
8. **Перевірка:** який тест міг би спростувати ваше правило?

Так elicitation збирає не лише supporting examples, а й decision boundary.
...
## Невизначеність: слова, частоти й імовірності
...
## LLM як секретар, а не носій повноважень
...
## Валідація: експерт перевіряє формулювання, світ — передбачення

Member checking потрібен: експерт підтверджує, що candidate коректно передає
його думку. Але це ще не незалежна перевірка rule. Сильніший контур:

1. **Source fidelity:** candidate відтворює слова й artifacts сесії.
2. **Triangulation:** rule зіставлено з logs, documents, tests або іншими roles.
3. **Counterexamples:** знайдено boundary cases і defeaters.
4. **Operationalization:** усі cues вимірювані, units і time windows визначені.
5. **Held-out cases:** knowledge engineer не добирає лише знайомі успіхи.
6. **Prospective/shadow use:** система прогнозує до того, як outcome розкрито.
7. **Governance:** owner, validity interval, review trigger і rollback визначені.
...
## Практичний спринт elicitation

1. Обрати одне дороге рішення і 10–20 різнорідних historical cases.
2. Підготувати consent, ACL, retention та non-disclosure boundaries.
3. Провести коротке domain interview для словника й task decomposition.
4. Розібрати два критичні incidents за timeline; не показувати outcome завчасно.
5. Виконати think-aloud або simulation на новому case.
6. Застосувати triads/laddering до найбільш нечітких constructs.
7. Створити candidates зі source spans, context, exceptions і open questions.
8. Провести окрему challenge-сесію з контрприкладами.
9. Перевірити на held-out cases і незалежних artifacts.
10. Лише після review випустити rule; призначити owner і trigger повторної оцінки.
