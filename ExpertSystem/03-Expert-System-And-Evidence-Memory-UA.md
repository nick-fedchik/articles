...
```mermaid
flowchart TD
    accTitle: Основні частини експертної системи
    accDescr: Факти поточного випадку надходять у робочу пам'ять, машина виведення поєднує їх зі знаннями, механізм пояснення формує перевірювану відповідь, а система здобуття знань підтримує базу знань.

    Q["Запитання та факти<br>поточного випадку"] --> WM["Робоча пам'ять"]
    KAS["Система здобуття знань<br>збирання · перевірка · оновлення"] --> KB["База знань<br>факти · правила · приклади"]
    WM --> IE["Машина виведення<br>застосовує знання"]
    KB --> IE
    IE --> EX["Механізм пояснення<br>підстави · джерела · прогалини"]
    EX --> H["Людина<br>перевіряє та вирішує"]
    H -.->|"виправлення знань"| KAS
```
...
```mermaid
flowchart TD
    accTitle: Від першоджерел до перевірюваного рішення
    accDescr: Первинні джерела проходять перевірку походження й версій, після чого правила та модель формують рекомендацію; уповноважена людина приймає рішення, а повний запис зберігається для повторної перевірки.

    S["Первинні джерела<br>вимоги · тести · код · дефекти"] --> V["Перевірка<br>походження · версія · права"]
    V --> C["Знімок контексту<br>виріб · час · конфігурація"]
    C --> A["Аналіз<br>правила + пошук + модель"]
    A --> R["Рекомендація<br>підстави · припущення · прогалини"]
    R --> D{"Перегляд людиною"}
    D -->|"прийняти"| OK["Рішення"]
    D -->|"уточнити"| MORE["Додаткові дані або перевірка"]
    D -->|"відхилити"| NO["Відхилена рекомендація"]
    MORE --> S
    OK --> LOG["Журнал рішення<br>хто · коли · чому"]
    NO --> LOG
```
...
```mermaid
flowchart TB
    accTitle: Організаційна пам'ять як мережа пов'язаних знань
    accDescr: Рішення пов'язане з вимогами, кодом, тестами, дефектами, ризиками та вивченими уроками; кожний зв'язок веде до джерела, версії й відповідального.

    DEC["Рішення<br>що і чому обрали"]
    REQ["Вимога<br>що потрібно"]
    CODE["Код або конструкція<br>як реалізовано"]
    TEST["Тест<br>як перевірено"]
    DEF["Дефект<br>що пішло не так"]
    RISK["Ризик<br>що може статися"]
    LES["Вивчений урок<br>що використати знову"]
    META["Для кожного вузла й зв'язку<br>джерело · версія · власник · права"]

    REQ -->|"обґрунтовує"| DEC
    DEC -->|"визначає"| CODE
    REQ -->|"перевіряється"| TEST
    CODE -->|"перевіряється"| TEST
    TEST -->|"виявляє"| DEF
    DEF -->|"уточнює"| RISK
    RISK -->|"впливає на"| DEC
    DEF -->|"створює"| LES
    LES -.->|"нагадує під час нового рішення"| DEC
    META -.-> DEC
    META -.-> REQ
    META -.-> TEST
    META -.-> LES
...
```mermaid
flowchart TD
    accTitle: Як досвід перетворюється на активне знання
    accDescr: Подія породжує спостереження, експерт перевіряє причину й межі, знання публікується з версією, спрацьовує в подібній ситуації та оновлюється за результатом застосування.

    E["Подія<br>дефект · затримка · рішення"] --> O["Спостереження<br>що сталося"]
    O --> V["Перевірка експертом<br>причина · вплив · межі"]
    V --> K["Запис знання<br>правило · приклад · джерело"]
    K --> P["Публікація<br>версія · власник · права"]
    P --> T["Спрацювання<br>у схожій новій ситуації"]
    T --> A["Дія або рішення людини"]
    A --> F["Результат і зворотний зв'язок"]
    F -->|"підтвердити або виправити"| V
```
