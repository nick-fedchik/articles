...
> **Після статті:** розкласти кіберфізичну систему на контури з різними дедлайнами; визначити місце експертного міркування на edge і backend; спроєктувати деградований режим, межі повноважень, синхронізацію знань та вимірювану перевірку.
...
21](21-Expert-System-Continual-Learning-UA.md) — кероване навчання з результатів.
Тепер ці механізми треба помістити в замкнений контур керування фізичним світом.

## Кібернетика — не синонім кібербезпеки
...
стан і команди. Кібернетика — чи дає обраний контур потрібну поведінку попри
...
### Навіщо тут закон необхідної різноманітності
...
## Замкнений контур: спостерігати недостатньо
...
## Один контур має кілька часових масштабів
...
де $D$ — дедлайн саме цього контуру. Швидка нейромережа не рятує систему, якщо
...
## Що експертна система робить безпосередньо на edge
...
### Контракт спостереження важливіший за сирий score
...
## Дозволена дія: оптимізація лише всередині безпечної множини
...
## Апаратний і програмний front-end сенсорики
...
сенсор, помилкову rule base, викрадену identity чи весь контур керування.
...
## Backend: не віддалений мозок, а інший рівень міркування
...
### Коли цифровий двійник справді корисний
...
## Системи управління військами: знання між повідомленням і повноваженням
...
flowchart LR
    ...
    ES --> C2["Уповноважений контур управління"]
...
## Робототехніка: не плутати поведінку зі стабілізацією
...
## Виробництво: PLC керує процесом, експертний шар — знанням про нього
...
## Три домени — один принцип, різні повноваження
...
## Degraded mode проєктують до втрати зв'язку
...
## Safety, security і цілісність знань — три різні перевірки
...
## Як вимірювати кібернетичний контур

Accuracy окремого classifier не відповідає на питання, чи система керована.
...
### Знання, пояснення й відновлення
...
## V&V: перевіряти треба систему, а не лише модель

[Стаття 19](19-Expert-System-Knowledge-Base-Verification-UA.md) описала
верифікацію rule base й inference runtime. Для кіберфізичного контуру додаються:
...
10. Додати backend aggregation, simulation та candidate learning лише після
    стабільного локального контуру.
...
## Антипатерни, які виглядають сучасно
...
Справді сучасною є не система з найбільшою моделлю, а кіберсистема, яка знає
...
- Який контур у вашій системі має найкоротший реальний deadline?
