# Кібернетика XXI століття: як експертна система працює від датчика до центру керування
...
> **Рівень:** розробники, інженери та технічні керівники: середній  
> **Після статті:** пояснювати, які рішення мають залишатися біля обладнання, які можна передати до центру, і як система зберігає безпечну поведінку при втраті зв’язку.

На конвеєрі датчик вібрації помічає незвичний шум двигуна. Чи треба негайно
зупиняти лінію, попросити людину перевірити кріплення або лише зберегти
спостереження? Відповідь залежить не від одного числа з моделі: важливі якість
вимірювання, час події, режим машини, наслідок зупинки та право системи діяти.

Якщо зв’язок із центром зник, частина міркування мусить лишитися біля машини.
Водночас локальний вузол не бачить історії всього виробництва і не повинен
самочинно отримувати повноваження технолога чи оператора. Центральне питання
статті: як розподілити спостереження, пояснення й дію між датчиком, локальним
вузлом і центром так, щоб рішення було своєчасним та підзвітним.

Це навчальна стаття, а не готовий проєкт для робота, лінії чи системи
управління військами. Допустимий ризик, часові межі, сертифікацію та права на
дію встановлюють для конкретної сфери. Військовий приклад тут обмежено
ситуаційною обізнаністю, координацією, логістикою, технічною діагностикою та
людським контролем — без тактики ураження або цілевказання.
...
21](21-Expert-System-Continual-Learning-UA.md) — обережне навчання з результатів.
Тепер ці механізми треба помістити в цикл керування фізичним світом.
...
стан і команди. Кібернетика — чи дає обраний цикл потрібну поведінку попри
...
## Як рішення повертається до фізичного світу
...
## Чому різним рішенням потрібен різний час
...
## Що має вміти локальний вузол

Локальний вузол працює біля пристрою й мусить ухвалювати рішення в межах
...
### Чому одного показника моделі замало
...
[статті 06](06-Expert-Systems-Architecture-UA.md) має зберігати список
джерел, а й залежності між ними.

## Як обирати дію в межах безпеки
...
## Як розподілити роботу між обладнанням і програмами
...
сенсор, помилкову rule base, викрадену identity чи весь цикл керування.
...
## Що робить центральний рівень системи
...
## У системах управління військами дані не дорівнюють повноваженню
...
flowchart LR
    ...
    ES --> C2["Уповноважена ланка управління"]
...
доступне лише завтра. Результатом є доказовий варіант
...
Робот, виробнича лінія та система управління військами відрізняються ціною
помилки, доступними даними й правом на дію. Проте в усіх трьох випадках корисно
відокремити швидке фізичне керування від пояснення, діагностики та ухвалення
рішення людиною.
...
Таблиця не пропонує переносити практику з одного домену в інший без перевірки.
Вона вчить ставити однакові запитання: що саме спостерігаємо, хто має право на
дію, який режим залишається доступним без зв’язку та яку помилку система не
має права приховати середнім показником якості.

## Як система поводиться без зв’язку з центром
...
Наприклад, локальний вузол може продовжувати збирати дані й радити додаткове
вимірювання, але не змінювати режим обладнання, якщо дані центрального рівня
застаріли. Така межа має бути записана до першої втрати зв’язку, а не
народжуватися з припущень оператора під час інциденту.
...
Отже, режим без зв’язку — це не «зменшена версія» звичайної роботи. Це окремо
перевірений стан із власними дозволами, журналом і способом безпечно
повернутися до спільної картини після відновлення каналу.

## Чому безпека дії, захист і точність знань не можна змішувати
...
## Як вимірювати цикл керування

Точність окремого класифікатора не відповідає на питання, чи система працює
передбачувано.
...
- хибні спрацьовування й пропуски для ризикових груп випадків, а не за однією
  загальною оцінкою;
...
## Як перевірити всю систему, а не тільки модель

[Стаття 19](19-Expert-System-Knowledge-Base-Verification-UA.md) описала
верифікацію rule base й inference runtime. Для кіберфізичного циклу додаються:

- **Перевірка на моделі процесу** — симулятор подає сценарії оцінювачу стану,
  моделі та правилам.
- **Перевірка програмного складу** — тестується реальний software stack і часові
  контракти без hardware.
- **Перевірка з обладнанням у циклі** — реальний controller/edge hardware працює з
  модельованим процесом і fault injection.
- **Тіньовий режим** — система будує рішення на реальних даних без side effects.
- **Обмежене польове випробування** — вузький operational scope, human gate, telemetry
...
## З чого почати невеликий безпечний проєкт
...
Спочатку описують змінні стану, спостереження, дії, збурення та повноваження.
Потім відокремлюють короткий дедлайн локального керування від задач, яким можна
чекати, і забороняють у швидкій частині недетерміновані компоненти. Контракт
спостереження має містити два часи, оцінку якості та походження даних; оцінювач
стану повинен уміти явно сказати «невідомо».

На цій основі достатньо почати з 10–20 локальних правил діагностики й режимів
та перевірок на суперечність. Для кожної рекомендації формують пакет доказів і
пояснення за [статтею 16](16-Expert-System-Explanation-Engine-UA.md), описують
множину $A_{safe}$ і обмежуються діями читання або чернетками рівня A0/A1 зі
[статті 18](18-Expert-System-From-Recommendation-To-Action-UA.md).

Після цього створюють підписаний пакунок знань з атомарним увімкненням і
відкатом, тестують втрату зв’язку, застарілі, дубльовані й суперечливі події.
Лише стабільний локальний цикл доповнюють агрегацією на центральному рівні,
моделюванням та навчанням кандидатів; наступне розширення повноважень можливе
тільки після окремого випуску з доказами.
...
## Рішення, що здаються сучасними, але створюють ризик

Найнебезпечніші архітектурні помилки часто маскуються під швидке впровадження
нової технології. Їх легко впізнати за обіцянкою, що один компонент замінить
перевірку даних, межі повноважень і план відновлення.

| Помилкове рішення | Чому небезпечно | Краща межа |
|---|---|---|
...
Спільна причина цих помилок — змішування різних ролей в одному непрозорому
блоці. Модель, правило, виконавець і людина можуть співпрацювати, але для
кожного має бути видно вхідні дані, межу відповідальності та спосіб зупинки.

Перед вибором нової технології варто запитати не «чи вона сучасна», а «яку
помилку вона може зробити, хто її помітить і що станеться за відсутності
мережі». Це повертає розмову від рекламної назви продукту до перевірюваної
інженерної властивості.
...
Прочитавши статтю, читач може розкласти складну кіберфізичну систему на
датчики, локальне керування, експертне міркування та центральний рівень;
пояснити, чому ці частини мають різні строки реакції й повноваження; а також
перевірити, чи має система безпечний режим без зв’язку, доказ для рекомендації
та спосіб повернутися до попередньої версії знань.

## Питання до читачів

- Який цикл у вашій системі має найкоротший реальний дедлайн?
